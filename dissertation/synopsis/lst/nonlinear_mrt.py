...
def __subs(sym_f, subs):
    ...
    Parameters:
        sym_f --- symbolic function to evaluate, for example: (y0 - y1)/(x0 - x1)
        subs --- dict of symbolic/numeric substitutions,
            for example: {x0: 8.98, y0: 8.98, x1: 9.31, y1: 9.31}
    Return:
        computed value as the result of numeric parameter substitutions
    """
...
def __gen_equations(delta_expression, values, num_params):
    """
    Provides system of equations from delta_expression for every parameter and
    list of symbolic substitutions for each equation.

    Parameters:
        delta_expression --- sympy object, which represents equation,
            for example: 'a * exp(-alpha*x) - y', which has to be equal to 0
    ...
    Return:
        ...
        f_subs --- list of dicts with of indexed symbolical_values, keyed by these symbolical values,
...
def __gen_subs(f_subs, values):
    ...
    Parameters:
        ...
        values --- dict of values, keyed by sympy objects
            for example, {x: [x1, x2, ...], y: [y1, y2, ...])
    Yield:
        res --- dict of substitutions, for example: {x1: 12, x2: 21, y1: 43, y2: 34}
    """
...
def search_mrt(delta_expression, parameters, values, err_stds):
    ...
    Return:
         computed estimates, computed estimates of symbolic parameters
    """
    ...
    sym_G = []
    # compute derivarives of parameter expressions, and place into matrix, for example:
    # G = [
